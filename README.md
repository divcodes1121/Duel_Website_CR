...
| **The console, the roster and the player's page share one dashboard shell — a sidebar that opens, minimises and closes — and their charts are Recharts** | **2026-09-27.** Asked for directly: the three screens were "plain"; the ask was a left sidebar with open, close and minimise, a palette for both themes taken from the best admin dashboards, and charts that are not boring. `ui/dash-shell.tsx`: a 264px sidebar that minimises to a 72px icon rail (each item names itself on hover and focus) and closes to nothing, remembered per screen; an off-canvas drawer below 64rem; Ctrl/⌘+B; a closed panel leaves the tab order. **The console is seven views with their own URLs** — Overview, **Tracking (new)**, Collection, Storage, Rollup, Site & domain, Accounts — and the sidebar carries each section's verdict as a dot, from the same thresholds its cards use (`state/consoleHealth.ts`): live it flags Rollup (coverage 45.7%, last full rebuild 23 July) and Site & domain (one integration missing). **Tracking** answers who was queued for collection, from which screen, and whether the bot has them: the bot's additions per day stacked by where each player was asked for (an "Asked on the site" tab re-scales past the leaderboard's 400-570-a-day bulk runs), requests by screen, the median wait from request to enrolment (59 min live) and every request with its state. Server side the queue now KEEPS what a prune used to delete (`tag_history`), and the admin-gated `/api/analytics/admin/tracking` reads it — route count 25. Charts are Recharts v3 (SVG, so the CSS tokens colour them in both themes; a keyboard layer built in; the renderer shadcn's and Tremor's charts use) on a six-colour series palette run through the dataviz validator for both themes (worst adjacent colour-blind ΔE 11.3); every chart has a table view, and the coach's day chart is TWO synced charts instead of a dual axis. 101/101 browser checks on real production payloads (64 console, 26 roster, 11 `#/my`), both themes, 1440 and 390. Main bundle −11.3 kB gzip (the console left it); the kit and Recharts are a 126 kB gzip lazy chunk that only these three routes load |
| **The dashboard kit is TailAdmin's now, and it answers back** | **LIVE 2026-09-26 (`2780326`).** Chosen by screenshotting 29 live demos from TailAdmin's list of 33 free admin templates in both themes (17 have no dark mode at all). The one kit behind the console summary, the roster overview, a player's coach dashboard and the player's own `#/my` was rebuilt on TailAdmin's measured layout (16px cards, 24px pad, 48px icon tiles, 30px figures, a half-arc gauge on a band of figures, segmented tabs) in this site's own tokens — TailAdmin's navy dark mode is not copied, and its grey labels are not either, because `--text-muted` is full ink by design. **Every chart mark has a tooltip that works by hover, tap and keyboard**, the hovered bar keeps its colour while the rest dim (text never dims), stat cards carry sparklines of the day chart's own series (the win-rate line breaks on days under 3 battles, exactly as the chart does) and change pills from `progress` ("within noise" when the server calls the windows indistinguishable), and "What they face" has a Share / Win rate tab that only draws rates past the 10-battle floor. It also fixed a counting bug: "Losing matchups" read the three-card list and so could never say more than 3. Layout is by container query, so it follows the box it is in, not the window. 76/76 browser checks on real production data (#YYPCUUY0, 487 battles), both themes, 1440 and 390, reduced motion; 970 vitest. Main bundle +3.44 kB gzip |
| **One dashboard, two screens** | **SUPERSEDED 2026-09-26 by the TailAdmin kit in the row above** — same file, same exports, now four screens. **LIVE 2026-09-20 (`762a8b6`).** The admin console's summary and the Coach Roster's overview are the same layout now — a hero figure, a key-metric grid, two charts and three tinted insight cards — ported by hand from the `bionis-dashboard` composition into CSS Modules, since this project has no Tailwind and the item is not in the public registry. **The ring is never a score**: on the roster it shows how many active players have nothing outstanding, prints a fraction rather than a percentage, and requires a caption saying what it counts. The console's operational sections are deliberately untouched. Verified 37/37 in a browser, both themes, 1500 and 390 |
...
| Export PDF | one button per screen, every tab, plus the full player report — rebuilt 2026-09-26 (the print-exact export is gone) |
| Opponent Intelligence Engine | **research CLOSED, model FROZEN**, flagged off (`CLASH_OIE=off`) |
| OIE prediction timestamp | **fixed and committed (`a9cdbb7`, pushed), NOT DEPLOYED, 2026-09-15.** A live read built its change-model input with the placeholder stamp `"9999"`, which does not parse, so features 9 and 10 (hours since the last deck change, hours since the last play) were always zero. `predictor.predict` now uses the caller's `cutoff_ts`, or the request clock in UTC in `battle_time`'s own format. An input correction, not a retrain or recalibration: weights, feature order, calibration cuts, caps and the candidate generator are untouched, and the primary (most recent) deck is identical under any stamp. Validated in Brain Phases 8, 8b, 8c and 10 (competitive AUC 0.619 -> 0.673, Brier macro 0.371 -> 0.275) at a known, accepted cost of fewer shown alternatives (alternative hits 1.85% -> 1.06% of reads); implemented in Phase 11 and proved equal to the validated condition offline on 357,426 reads with 0 mismatches. Feature version `phase2-21` -> `phase2-21-reqstamp-utc`. Brain Phase 20 replicated the gain on a later window (AUC +0.022, Brier macro -0.046, gate 8/8) and found 8b's headline inflated by a post-purge re-store artifact. **x9 override added locally, 2026-09-19 (uncommitted, NOT DEPLOYED).** The served model now reads feature 9 (hours since the last deck change) as 0; feature 10 stays live from the stamp. Brain Phase 20 tested it on a later window than the one that suggested it: macro Brier -0.0137 [-0.0186, -0.0089], ECE 0.182 -> 0.142, ROC-AUC -0.0029 (inside the -0.010 margin). An input override on the frozen artifact, not a retrain; proved equal offline to the validated arm on 357,426 + 196,781 reads, 0 mismatches. Feature version `phase2-21-reqstamp-utc` -> `phase2-21-reqstamp-utc-x9zero`. **The VPS still runs the old stamp and `CLASH_OIE` stays `off`**; a dark deploy (Brain Phase 13, still `off`) is a separate approval, and `shadow` and `on` are separate again. See `DECKKIES_BRAIN_README.md` |
...
`#/admin`. Seven views in a sidebar since 2026-09-27 — the section below
describes that first; the one-screen history that follows it is kept because
every figure and threshold in it survived the move.

### A sidebar, one view per section, and the verdicts on the sidebar (2026-09-27)

The console was one long page of tiles — "plain", in the account holder's
word, and a health check meant scrolling past everything to the one section
being checked. It sits in the dashboard shell now (`ui/dash-shell.tsx`, shared
with the Coach Roster and the player's own page), and every section is a view
with its own URL, so a refresh or a pasted link lands where it was:

| view | route | what it holds |
|---|---|---|
| Overview | `#/admin` | the collection gauge, accounts / paid / signed-in / waiting cards, the last 14 days of additions, accounts by tier, capacity, the recruiter and the card catalogue |
| Tracking | `#/admin/tracking` | who was queued for collection, from which screen, whether the bot has them, and how long that took — new, see below |
| Collection | `#/admin/collection` | newest battle, last poll, poll failures, battles stored, the roster |
| Storage | `#/admin/storage` | data against free pages inside `battles.db`, the volume gauge, pages and the retention runway |
...
the volume fills or the retention runway is short, Rollup when coverage drifts
or the rebuild is overdue, Site & domain when the API is down, the card
catalogue is missing or an integration is unset. The thresholds live ONCE, in
`state/consoleHealth.ts` (no imports, 17 tests), read by both the dots and the
cards, so a sidebar can never call a collector healthy while its own card says
Stalled. **Only warnings and faults get a dot**: a wall of green dots is a
dashboard shouting, the rule the Integrations tile already followed. Live, on
the day it was built, the dots were Rollup (coverage 45.7%, last full rebuild
23 July) and Site & domain (`oieAllowlist` unset).

**NOTHING WAS DROPPED IN THE MOVE.** Every figure, note and threshold the old
tiles showed is on a card now, with its reasoning comment beside it in the
source; the poke-ripple tiles and the console's own LiquidMetal canvas went,
because the cards are the kit's and the kit does not do that.

**TRACKING, THE NEW VIEW.** The queue is empty most of the time — the bot
drains it every two hours — so the one figure the console had ("16 queued")
was almost always a count of rows already collected. The view answers the real
questions from two tables: the bot's `tracked_players.added_at` (when each
player arrived) and the queue plus its new history (`tag_history`, which keeps
what a prune used to delete, source and request time included). It shows the
bot's additions per day stacked by where each player was asked for — six
families (Player search, Analytics screens, Coach & teams, Leaderboard, 2v2,
Opponents) plus "Other", the enrolments with no queue record — an
**"Asked on the site"** tab that re-scales the axis to the three site families
(one leaderboard day adds 570, a search adds one), requests by screen in their
family's colour, the median wait from request to enrolment, and every request
with its state. Whole UTC days, 7 / 30 / 90. `botRead: false` is said on
screen rather than presented as "everything is waiting".

**THE CONSOLE IS LAZY NOW.** It was imported eagerly, which put the whole kit
in the main bundle; with Recharts behind it that would have been ~100 kB on
every public page. `React.lazy` in `App.tsx` took the main bundle from 356.10
to 344.79 kB gzip.

**IT ALSO WAITS FOR THE ACCOUNT BEFORE JUDGING IT.** `ready` is true before the
profile arrives and the tier defaults to free, so an admin used to see "Not
your console" for a beat on every load — the flash the roster had already
fixed. The console uses the roster's `resolved` test now.

### Before 2026-09-27: one screen
...
**The PDF matches the screen now (2026-09-26).** Every report embeds Bebas Neue
for titles and figures and Inter for everything read, both subset to Latin and
the punctuation the adapters use by `scripts/build-report-fonts.py` (Inter 410
-> 24 kB a weight, Bebas 59 -> 18 kB), with `src/utils/report/glyphs.json`
recording what each subset actually holds. It was possible for exactly the
reason the table below records: Bebas Neue is TrueType.
...
The builder's deck report used to keep `Subscribe.ttf` for this reason; since
2026-09-26 it is drawn by the same engine in Bebas Neue and Inter, and
`Subscribe.ttf` is no longer read by any PDF.
...
### Every tag asked about anywhere is queued (2026-09-26)

**Asked for by name: a tag searched or added anywhere goes into the database
and is polled from the next poll.** Checked route by route, and it was not
true. Only the player search, `/track`, `/live` and Team Analysis queued an
untracked tag; **Duel Analysis, Duel Zone, Recent Battles, Cards, Deck
Counter, all three Coach Assist reads (prediction, suggestion — both players —
and the opponent read) and the Coach Roster's intel and field plan read a tag
and forgot it.** So a player reached by a shared `#/player/<tag>/duels` link, a
Coach Assist opponent or a roster read was never collected. And a tag saved in
**Supabase** — a player's own tag in onboarding or the profile — never reached
the analytics server at all (the roster's Add Player already called `/track`).

**THE CONSOLE CAN SEE IT NOW (2026-09-27).** Queueing every tag was only half
of it: `prune_enrolled` DELETED a row the moment the bot had it, so which screen
had asked, and when, was gone within a poll — the console could say how many
tags were waiting and nothing about who had been queued. The prune MOVES the
row into `tag_history` now (same columns plus `pruned_at`, aged out after
`HISTORY_DAYS` = 120), and `tracking.activity(days)` joins the queue, the
history and the bot's own `tracked_players.added_at` into the console's
Tracking view: each request's state, its wait, the bot's additions per day by
source (`direct` when no queue row exists), the median wait. Read through the
admin-gated `/api/analytics/admin/tracking`, because it lists tags people
looked up. The first live reading: 16 requests in 30 days, all collected, a
median of 59 minutes from request to enrolment — the next poll, as designed.

- **Every tag route calls `_note_tag(tag, source)`** in `server/app.py`, right
  after the tag is validated (and after the admin check on the roster intel
  route, so an unauthenticated request cannot write the queue). It is
  memoised for ten minutes so paging and roster-wide reads do not re-check
  the bot's table. `source` records which screen asked.
- **`saveProfile` queues the player's own tag** (fire-and-forget, like Add
  Player) — onboarding and the profile editor both go through it.
- **A searched tag always makes the next poll.** The bot drains
  `CLASH_TAG_DRAIN_BATCH` (2000) rows oldest-first at the START of each poll,
  enrols them and collects them in that same pass (`poll_tracked_players`
  drains before it reads `tracked_players` — checked in `bot.py`, not
  changed). A bulk backlog bigger than one batch would have pushed a search
  past its poll, so bulk sources (the leaderboard recruiter, 2v2 promotion)
  now fill the queue only to `BULK_QUEUE_CAP` = three quarters of the batch;
  the rest is reported as `deferredForSearches` and queued on a later run.
- **Proven on production**: an opponent tag from a real battle log, untracked,
  was queued by one `/duels/` read (pending 8 -> 9). `server/test_enrol_routes.py`
  (28 checks) fails if any route that normalises a tag stops queuing it — it
  is what caught `/coverage`, which the first pass missed.
...
note cut to one factual line (the no-prose rule that governs the Team Scout
screens), and the forced page break before each head-to-head removed — the new
packer keeps a heading with its first pair, so the break only left half-empty
sheets. A name the fonts cannot draw falls back to the tag. **Measured on a real
3 v 3 (six suggest-list players): 23–26 pages, ~1.15 MB, rendered in ~1 s,
14 ms a page to paint, audit clean** — against 37 pages at 3 v 3 before.

The table below is the 2026-09-06 engine's, kept for the record. Length follows
the rosters rather than a target. Measured, dark, with real card art:
...
The adapter is **dynamically imported**, and the export button's thunk (then `ReportButton`, now `ExportButton`) accepts a
...
**SUPERSEDED 2026-09-26** by the rewrite described in
[Exporting a screen as a PDF](#exporting-a-screen-as-a-pdf). The 2026-09-06
engine (`fit.ts`, `sections.ts`, `paint.ts`, `palette.ts`, variant scoring and
reflow) is deleted and lives in git history. What it taught is carried into the
new modules rather than repeated here:

- **Measure before committing.** Hardcoded height guesses (`case 'decks':
  return 34`) stranded headings and broke pages early. Every atom in
  `report/blocks.ts` is measured against the real embedded fonts first.
- **A heading travels with its first row** (`keep` in `report/pack.ts`), and a
  spill page repeats the heading as "continued" — landing on a sheet of rows
  with no heading is the most disorienting thing a paginated document does.
- **The audit reads what was committed**, never the layout's own arithmetic
  (`report/audit.ts`).
- **`charSpace` is in the document unit and applies after every glyph**;
  `getTextWidth` knows nothing about it. `Surface.width` adds tracking back and
  alignment is computed there, because jsPDF's own alignment ignores it.
- **Legibility floors are where an engine free to choose density stops** — and
  the rewrite added a ceiling, because the complaint was art drawn too large.

---

## Exporting a screen as a PDF

**Rebuilt from scratch on 2026-09-26.** Every PDF the site produces — the
analytics screens, the full player report, the Team Analysis dossier, 2v2
Decks and the builder's deck report — is drawn by one engine in
`src/utils/report/`, in one fixed dark Deckkies look, from a data model the
screen hands it. There is exactly **one Export button per screen**.

### Why it was rebuilt: the old export was a screenshot

The player shell's **Export PDF** used to be `window.print()` on the live page.
Chrome can only print `backdrop-filter` glass by rasterising it into
soft-masked bitmaps, and a PDF viewer composites every soft mask per pixel on
the CPU. Measured on production data, the same four screens exported both ways
in one run:

| | size | render per page (MuPDF, 110 dpi) | worst page | transparency states |
|---|---|---|---|---|
| **old** — shell `window.print()` | 2.5–8.6 MB | 175–229 ms | 308 ms | 60–263 |
| **new** — `report/` engine | 0.26–1.5 MB | 14–25 ms | 50 ms | **0** |

The print path was also broken on its own terms: the content below the fold
did not reach the PDF (pages of chrome and an empty panel), and the old jsPDF
button beside it printed a near-empty cover, a raw `20260926T0` under "Last
played", colliding column headers and a win-rate bar that was always full.
Two buttons labelled "Export PDF" did two different things.

### The engine: measure, pack, draw — and nothing transparent

| module | job |
|---|---|
| `geometry.ts` | A4 landscape, margins, the four-role type scale, legibility floors (`FONT_MIN` 5 pt, `CARD_MIN` 7 mm) **and a ceiling** (`CARD_MAX` 11.5 mm). No imports. |
| `theme.ts` | The fixed palette: the site's dark `--hue-*` and `--solid-*` values, copied, never read off the page. Win-rate colouring, the matrix ramp, contrast helpers. No imports. |
| `text.ts` | Every string filtered to the glyphs the embedded font really holds (`glyphs.json`), `printableName()` for player names. |
| `fonts.ts` | Inter 400/600 and Bebas Neue, fetched once a session, subset. Helvetica if the fetch fails. |
| `art.ts` | Every raster, baked **opaque** on a canvas: card tiles flattened onto the slot colour, the corner glow, the hero band and cover (the landing's castle backdrop and king), the logo. |
| `surface.ts` | Drawing primitives. Gradients are opaque bands inside a clip; buttons are gradient pills with a lit edge and a real link annotation. |
| `blocks.ts` | Each block kind measured into **atoms** — the units that may not split (a table row, a row of deck cards, one duel series). |
| `pack.ts` | Greedy pagination: keeps a heading with its first row, reserves a "continued" heading when a block spills, opens sections on a new sheet. No imports. |
| `audit.ts` | Runs on every export, reading the boxes actually committed: type floor, card floor and ceiling, body bleed. |
| `engine.ts` | The pipeline, the cover or hero band, the brand bar, the footer, the contents page and the viewer's bookmark outline. |

**NO TRANSPARENCY STATE ANYWHERE.** Muted colours are real colours, glows are
baked into images, the ghost crown is a pre-mixed opaque fill.
`tests/reportLayout.test.ts` fails if a `GState` or an `opacity` appears in
any engine module — that is the lag, and it is the one regression this rewrite
exists to prevent.

**THE PAGE GROUND IS A VECTOR FILL, NOT AN IMAGE.** The first cut baked a
full-page background image and measured 56–68 ms a page; a solid fill plus a
150 x 96 mm corner glow brought it to the 14–25 above. Card tiles went 150 ->
120 px on the same measurement (still ~300 dpi at the largest card drawn).

**ALWAYS DARK.** The old renderer read the live CSS tokens, so a reader on the
light theme got a white report. A branded document is the same file for the
same data.

**DECKKIES ON EVERY PAGE.** Logo tile and the DECKKIES wordmark with ™ in the
brand bar, the fan-content line on the cover and at the end. **No site URL**,
as before.

**CARDS IN SLOTS, AT THE SITE'S SIZE.** Each card sits in a rounded slot at the
art's true ratio; an evolution's slot takes a violet edge and a hero's an amber
one. The ceiling is enforced by the audit, because the complaint that started
this was art far larger than the website draws it.

**EVERY DECK HAS AN OPEN IN GAME BUTTON** (8 known cards), a gradient pill that
is a working `copyDeck` link in the PDF.

### One button, every tab, and the full player report

`state/reportExport.ts` is a registry. A screen declares its export with
`useScreenExport({ id, build, ready, win })` — a thunk that builds the whole
report, called at the click. Inside the player shell (`ReportHost`) the
declaration goes to the shell's button in the query row, which sits in the
same place on every section; anywhere else (Team Analysis, 2v2 Decks, the home
boards) the hook returns an inline button. `components/Export/ExportButton.tsx`
is the only button: spinner and step text while building, "Saved · N pages"
after, the failure stated.

The shell button is a menu: **This page** (the open section, every tab) or
**Full player report** (`utils/playerDossier.ts`): every section the account
may open — `sectionAllowed` is the one rule, so the dossier is not a way round
a gate — read in parallel over the window the open section shows, bound into
one document with a cover, a contents page, a section opener each, and a line
naming any section that could not be read.

**The button used to vanish below 1080px.** It shared `.topSeason` with the
season picker, and the whole slot was hidden on narrow screens, so tablets and
phones had no export at all. Only the picker is hidden now.

| screen | exported |
|---|---|
| Player Analysis | headline figures, **every** deck (was top 10), share of play, both trend charts |
| Live Player | as before |
| Recent Battles | the 50 most recent battles in the window (the server's page ceiling), modes not shown |
| Duel Analysis | all three tabs **and the Duel Insights panel** (fetches Duel Zone at the click) |
| Duel Zone | series log (cap 24 -> 60) and deck sequence |
| Cards | a card-art grid per tab: Troops/Buildings/Spells (together the All tab), Win Conditions, Champions, Evolutions and Heroes in their own art |
...
`state/printMode.ts` are **deleted**, and Duel Analysis no longer has a print
mode.

### Things that went wrong while building it

- **EVERY EXPORT FAILED IN A TAB OPENED BEFORE A DEPLOY, AND A README-ONLY
  COMMIT WAS ENOUGH (found in production minutes after shipping).** The commit
  id printed in every PDF footer was compiled into the lazy report-engine
  chunk through Vite's `define`, so every commit renamed that file
  (`engine-CchoGdN4.js` -> `engine-Dl7MrkWw.js` on a docs push), and Vercel
  serves only the newest deployment's files. A tab opened two minutes earlier
  asked for the old name, got a 404, and every export on every screen said
  "Export failed". **Fresh page loads always worked**, which is why a
  signed-out production check passed while the account holder's tab failed.
  The id now lives in a `<meta name="deckkies-build">` that `vite.config.ts`
  writes into `index.html` (never a hashed asset); two builds with different
  commit ids now produce **identical names for all 39 JS/CSS files**.
  `state/staleBuild.ts` makes a REAL code deploy recoverable: a failed lazy
  load during an export shows "Deckkies was updated since this page opened"
  with **Reload to export** (not an automatic reload — an unsaved Team
  Analysis board is on that page), and a failed lazy SCREEN reloads once
  (`vite:preloadError`, loop-guarded for 30 s). Verified 7/7 by deleting the
  engine and the field book's chunk from under an open production build.
  `tests/staleBuild.test.ts` fails if a per-commit value returns to `src/`.
  **This was latent before the rebuild** — the old engine printed the build id
  too, so every deploy since then had broken exporting in open tabs.

- **A colour rule that assumed every percentage was a win rate** printed
  opponents' 13% USE rates red on every Team Analysis page. A figure is
  coloured as a rate only when it is one (`NOT_A_RATE` in `blocks.ts`), and a
  deck may carry `valueHue` for a meaning the rate alone does not state: a 63%
  "worst matchup" is nine points under the player's own average and is red.
- **The matrix lost its stretched scale in the rewrite** and painted all nine
  cells one green (68–75%). The stretch lives in `matrixAtoms` now.
- **Filtering a name to drawable glyphs is not enough.** "Потужнi лававод"
  kept its single Latin "i" and printed as "I". `printableName()` keeps a
  name only when most of its letters survive, and the tag stands in otherwise.
- **The win-rate trend dived to 0% on every day a deck was not played** — the
  server sends 0 there. It is a gap now, and the chart is six lines, not ten.
- **Gradient bands were visible** at 8 per bar and then at 2.5 mm each; 0.8 mm,
  one per colour level at most, is invisible and still ~20 ms a page.
- **Probe traps**: a PDF's text extracts with letterspacing, so search it with
  whitespace stripped; and `button:has-text("Export PDF")` also matches the
  label while it reads "Building PDF…".
...
them hit (Brain Phase 8c). **Feature 9 is then served as 0** (added 2026-09-19, uncommitted): it is still
computed, but the model scores `predictor._served_vector`, a copy with `log_hours_since_change`
zeroed, because Brain Phase 20 measured it hurting calibration on new data (macro Brier -0.0137,
AUC unchanged within -0.010). The feature version is now `phase2-21-reqstamp-utc-x9zero`,
...
**IN THE DASHBOARD SHELL SINCE 2026-09-27.** The roster IS the sidebar now —
Overview, then every active player as an item with a two-letter avatar in a
colour that follows the player, "Add player" under them, the archived players
in a folded group — and it opens, minimises to a rail of avatars (each names
its player on hover) and closes, like the console. On a phone it is the drawer,
which is why the narrow-screen player dropdown is gone. The top bar carries the
player's name and section; the roster overview's "Nothing here is a rating of
a player" moved into its subtitle, because the page no longer opens by saying
its own name twice. The Overview tab's day chart is two synced charts now —
battles per day stacked by result, then the win rate against the window's own
...
### Phase 2: the player intelligence (live 2026-09-20, `ed04b98`)
...
**Checked in production after the deploy**: the VPS copies match the commit;
no token, a garbage token and a forged three-segment JWT all answer 401 — the
forged one only after Supabase itself rejected it, which proves the service
reads its Supabase settings and can reach it; the CORS preflight from
...
### Phase 3: the deck arsenal (live 2026-09-20, `a1ac2e0`)
...
### Phase 4: the opponent scout (live 2026-09-20, `7f54bb2`)

`#/admin/coach/<PLAYER>/scout/<OPPONENT>` — what the person on the other side
actually plays. Reached from the Scout tab, which offers the opponents this
player keeps meeting, or from any row of the Opponents tab.

**It reads and stores nothing**, and there is no migration for this phase. What
a coach concludes about an opponent belongs to the match plan they write next;
a cache of other people's habits is not something this project needs to keep.

**TWO FOOTINGS, AND THE SCREEN NEVER BLURS THEM.** A scouted opponent is
usually not on the roster, so the evidence is one of two very different
things, and the badge says which: **Stored history**, the same own-deck 1v1
counting the rest of the workspace uses, or a **Live snapshot** — the ~25
battles the game will hand back right now, which is this week and not a
record. A third answer, *Nothing stored*, is real and says outright that
nothing below is a claim about how they play. The footing is decided by
`scoutBasis`, never inferred from a list happening to be empty, and the two
kinds of deck list are never mixed in one view.

**HEAD TO HEAD IS THE ROSTER PLAYER'S RECORD, AND EVERY LABEL SAYS WHOSE.**
The one misreading that would matter here is taking the opponent's own win
rate for how they do against *your* player, so the tiles are named — "Rahul
won", "Rahul lost", "Rahul's win rate here · against this opponent only" —
and sit apart from "Their win rate". Not having met is stated as no record at
all rather than drawn as an even one, and a rate is withheld under three
meetings.

**Nothing is claimed that the counts cannot carry**: a deck win rate is
withheld under five battles, the archetype they lead with is named only past a
real share and only on non-thin evidence, and there is no confidence
percentage anywhere — the same rule the prediction engine's own policy
applies. A browser check asserts that last one on every footing.

**Two faults found by the browser run, both fixed rather than worked around.**
The scout waited for *both* reads before drawing anything, which made it as
slow as its slower half — and for an uncollected tag the profile read goes out
to the game's own API, so it could sit on a loader for tens of seconds; it now
draws as soon as the intelligence answers and lets the profile fill in behind.
And `ShareBars` laid out a fill wider than its track when handed a total from
a different count: clipped on screen, so it would never have been noticed, and
wrong. The width is clamped.

Verified **32/32** in a browser across all three footings, both themes and at
390px. Coaching chunk 19.02 → 21.55 kB gzip; main bundle unchanged.

**Checked in production after the deploy**: the served chunk carries every
scout string; the admin intelligence route answers 401 for an arbitrary
opponent tag with no session, so the gate holds for tags that are not on
anyone's roster; and both footings really exist out there — a tracked tag
answers `basis: stored` with its decks, an invented one answers `basis: live`
with nothing in it, which is the case the screen calls "Nothing stored".

**A SCOUT ENQUEUES THE TAG FOR COLLECTION, and that is the player report's
existing behaviour rather than anything this phase added** — the same thing a
player search has always done. It is worth knowing rather than fixing: an
opponent scouted today may have stored history the next time you look, which
is the direction a coach wants. The queue's own ceiling and pruning still
apply.

**Not built, deliberately:** no recommendations, no match plans, no
...
**IT RANKS NOTHING ITSELF, AND THAT IS THE DESIGN.** `team_analysis.py`
already answers this question: a spread-weighted expected win rate against
what the opponent actually brings, how much of their play that figure covers,
a practice tiebreak, and a named reason when it can say nothing. It is the
same engine the public Team Analysis screen uses, it is tested, and it is
deployed. A second ranking living in the Coach Roster would be a second
opinion that eventually disagrees with the first about the same two players.
This screen calls it with one blue tag and one red tag and presents what comes
back, unchanged — the figures on screen are the engine's own to one decimal.

**WHAT THE PHASE ADDS IS THE JOIN.** The engine's candidate pool is the decks
the player HAS PLAYED; the arsenal is the decks the coach has APPROVED. The
useful questions live in the overlap: suggested and approved (the strongest
thing the screen can say), suggested but not approved (one click to approve,
with the source recorded), and approved but not suggested — where the screen
distinguishes the two cases that look identical and mean opposite things:

> **Scored, but not among the top picks** — evidence against the deck.
> **No stored battles, so the engine cannot score it** — missing evidence,
> not a verdict.

**THE ARSENAL KEEPS THE COACH'S ORDER.** Re-sorting it by expected win rate
would quietly replace their judgement with a model's, which this project has
refused since phase 1. Suggestions are marked, never moved.

**Approving a suggestion writes down why.** `source: 'coach_assist'` plus a
`source_ref` holding the opponent, the window and the engine's own figures —
so a deck added today because it was expected to win 67.7% against one
opponent still says so in a month, when the meta has moved.

Verified **24/24** in a browser against a REAL engine response captured from
production for one real pairing (three picks at 67.7 / 59.5 / 56.2% expected,
147–165 practice games), replayed rather than hand-written. Both themes, 390px.
Coaching chunk 21.55 → 23.41 kB gzip; main bundle and CSS unchanged. No
migration, no server change.

**One probe lesson worth keeping:** the first screenshot of this screen showed
two suggestion rows with almost no card art, which looked like a real fault
...
### Phase 6: match plans (live 2026-09-20, `7cca499`)

A plan is one preparation against one opponent: what the player leads with,
what they fall back to, and what is held in reserve. Three slots, one deck
each — a "backup" that is three decks is not a backup, and the table's unique
`(plan_id, slot)` says so rather than the screen hoping.

**No migration.** 004 already defined `coach_match_plans` and
`coach_plan_decks`, down to the draft / confirmed / closed status, the notes,
the test-mode flag and the per-slot source.

**THE SNAPSHOT IS WHAT MAKES IT A PLAN RATHER THAN A NOTE.** Every plan keeps
its own frozen copy of what the screen was showing when it was drawn up —
each candidate, its source, its evidence — plus which engine and window
produced the ranking. The engines move on: the matchup snapshot is rebuilt,
the window rolls forward, a deck's expected rate changes. Without the freeze,
"why did we bring this" is unanswerable within a week, and phase 7 would have
nothing to compare an outcome against. The frozen block says so on screen:
*"Figures below are as they were then — the engines move on."*

**Both sources are frozen, each labelled.** A snapshot holding only the
engine's picks would make every plan look engine-driven in hindsight, which
is exactly the question the outcome phase exists to answer. An arsenal deck
the engine never scored is stored with `null`, never `0` — the screen reads
"never scored" rather than implying it scored nothing.

**Decks come from the snapshot or from the arsenal**, and the choice carries
its source into the plan. A deck the coach invents is added to the arsenal
first — where its own source records that it was manual — so decks are built
in exactly one place and the plan still records where its choice came from.

**Confirming needs a primary and nothing else.** A coach who has decided what
to lead with has made the decision the plan exists to record; requiring three
full slots would teach them to fill boxes. What is missing is said in words
rather than left as a disabled button.

Verified **27/27** in a browser, replaying the real production engine response
for a real pairing: the plan freezes four candidates from both sources, a slot
replaces rather than duplicates, confirming appears only with a primary, and
the frozen block names `team_analysis` and its window. Both themes, 390px.
Coaching chunk 23.41 → 27.37 kB gzip; main bundle unchanged.

**Checked in production after the deploy**: the served chunk carries the tab,
the save action, all three slot descriptions, the freeze warning, "never
scored", the confirm gap and the later-phase note. No migration and no server
change, so this shipped as one push.

**Not built, deliberately:** no results and no learning loop — the screen says
outright that the result belongs to a later phase.

### Phase 7: results and the learning loop (live 2026-09-20, `2ba3f68`)
...
### Phase 8: the roster overview (LIVE 2026-09-20, `e0b4a7c`)

`#/admin/coach` with no player selected — what a coach opens first. It used to
jump straight to the first player, because an empty frame was the only
alternative; there is a real screen there now, and the sidebar heading is a
link back to it.

**IT COUNTS; IT DOES NOT RANK.** Every column is something the coach did or
did not do — decks approved, plans drafted, matches recorded — and the order
is the roster's own, active first and then alphabetically, exactly as the rail
reads. A readiness score would invite comparing eight people with eight
different amounts of stored history, which is the one comparison this data
cannot support. Attention is shown by flags on a row, never by moving it up.

**EVERY FLAG IS A FACT WITH AN ACTION.** "No deck approved yet — approve the
decks they are ready to play, in Arsenal." "A confirmed plan has no result
recorded — record how it went, in Results." None of them is a remark about
the player, and a test asserts the wording carries no judgement. A flag with
no action is a nag.

**THREE QUERIES FOR THE WHOLE ROSTER, AND NO ANALYTICS AT ALL.** Row Level
Security already scopes the coaching tables to the coach, so "all my decks" is
one select of two columns. Fetching each player's battle intelligence to build
a summary would put one expensive scan per player behind the cheapest-looking
screen in the product. The "not collected" flag is therefore withheld rather
than guessed: the tracked set is not known here, and absence of knowledge is
not evidence.

**The floor is shared with the Results screen**, so the two can never disagree
about one player, and the screen says so.

Verified **20/20** in a browser: the bare route showing the roster, flags
appearing and then disappearing as the work is done, roster order holding with
a flagged player, and rates withheld. Coaching chunk 31.13 → 33.06 kB gzip;
main bundle unchanged.

**Two faults found on the way, both mine.** `.rosterName` was defined twice in
one CSS module — the sidebar's and the overview's — so both rules applied to
both elements; a class defined twice merges silently, which is the inverse of
the missing-class trap this file already records. And "0 matchs": appending an
"s" is not how English plurals work, so both forms are passed in now.
...
src/
  ...
  utils/
    ...
    analyticsPdf.ts           the engine's front door. A LAZY SHIM: nothing
                              runs until Export is pressed, so none of it is in
                              the bundle every reader downloads. `import()` in an
                              event handler, never React.lazy
    report/                   THE REPORT ENGINE (rebuilt 2026-09-26): measure,
                              pack, draw, audit. Always dark, and NOTHING in it
                              sets a transparency state -- that was the lag
      geometry.ts             page geometry, the four-role type scale, floors
                              and a card-size ceiling. No imports
      theme.ts                the fixed Deckkies palette (the site's dark hue
                              values, copied). No imports
      text.ts                 strings filtered to the glyphs the fonts hold;
                              printableName() for player names
      glyphs.json             written by scripts/build-report-fonts.py
      fonts.ts                Inter + Bebas Neue, fetched once, Helvetica fallback
      art.ts                  every raster baked opaque: card tiles, corner
                              glow, hero band, cover, logo
      surface.ts              drawing primitives; gradients are opaque bands in
                              a clip, buttons are gradient pills with links
      blocks.ts               each block kind measured into atoms
      pack.ts                 pagination, pure
      audit.ts                the page check, off the committed boxes
      engine.ts               the pipeline, cover, brand bar, outline
    screenAdapters.ts         battles, deck counter, coach, 2v2, global cards,
                              duel insights -- dynamically imported
    playerDossier.ts          the full player report: every allowed section
    pdfRenderer.ts            the builder's deck report, on the same engine
