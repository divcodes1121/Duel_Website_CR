...
function recLine(r: TeamRecommendation, opts: { showOwner?: boolean } = {}): DeckLine {
  const bits: string[] = [];
  if (opts.showOwner) bits.push(who(r.owner));
  bits.push(r.name);
  bits.push(`${int(r.comfort.games)} games at ${pct(r.comfort.winRate)}`);
  bits.push(`covers ${pct(r.spreadCovered, 0)} of their play`);
  return {
    name: opts.showOwner ? who(r.owner) : r.name,
    meta: bits.slice(opts.showOwner ? 1 : 1).join(' · '),
  ...
  if (picks.length) {
    blocks.push({
      ...
      rows: picks.map(({ f, r }): TableRow => ({
        ...
        deck: r.name,
        owner: { text: who(r.owner), hue: 'blue' },
        exp: {
  ...
  if (folder.recommended.length) {
    ...
    const pairs = folder.theirDecks.slice(0, 3).map((d, i) => ({
      ...
      note: folder.recommended[i]
        ? `${who(folder.recommended[i].owner)} flies ${folder.recommended[i].name} — expected ` +
    ...
    blocks.push({
      ...
      rows: report.blue.map((m): TableRow => {
        ...
        return {
          ...
          comfort: `${int(best.comfort.games)} games at ${pct(best.comfort.winRate)}`,
...
function teammateBlocks(
  ...
  blocks.push({
    ...
    rows: assignments.map(({ f, best }): TableRow => {
      ...
      const isLead = squadBest && best && squadBest.owner.tag === member.tag;
      if (!best) {
        ...
        return {
          ...
          lead: squadBest ? { text: who(squadBest.owner), thin: true } : { text: '—', thin: true },
        };
      }
      return {
        opp: { text: who(f.player), hue: 'red' },
        ...
        lead: isLead
          ...
          : squadBest
            ? { text: who(squadBest.owner), thin: true }
            : { text: '—', thin: true },
      };
    }),
  });
...
export function teamAnalysisReport(
  ...
  blocks.push(
    ...
    },
    {
      ...
      body:
        ...
        'their play it is. Every candidate deck from your squad is then scored as the sum over ' +
        'those archetypes of weight × win rate against that archetype. The win rate comes from ' +
        'the same matchup ladder the Deck Counter uses, unchanged: exact deck-versus-archetype ' +
    ...
    },
    {
      ...
      body:
        `A deck played forty times gains at most a point and a half of expected win rate over ` +
        `one played ${report.limits.minComfortGames} times, and nothing below ` +
        `${report.limits.minComfortGames} games enters the pool at all. That ordering is ` +
        'deliberate: the matchup comes first because it is what was asked, and practice breaks ' +
        'the tie between two decks that are inside the noise of each other. It is not a claim ' +
        'that repetition is worth a point and a half of win rate.',
    },
    {
      kind: 'note',
  ...
  const caveats = [
    ...
    'The candidate pool is only decks your squad has actually played. This report will never ' +
      'suggest the best deck in the game if nobody on the roster flies it, and that is the ' +
      'design rather than a limitation of the data.',
  ...
  return {
    screen: 'Team Analysis',
    subject: `${blue} v ${red} — ${report.folders.map((f) => who(f.player)).slice(0, 3).join(', ')}${
      report.folders.length > 3 ? ` +${report.folders.length - 3}` : ''
    }`,
    hue: 'pink',
    summary: 'A section for every player on both sides.',
    meta,
    blocks,
    caveats,
    contents: true,
