/* GlassDock — vengenceui, installed from the registry rather than retyped.
 *
 *   npx shadcn@latest add https://www.vengenceui.com/r/glass-dock.json
 *   registry: https://www.vengenceui.com/r/glass-dock.json
 *   file:     components/ui/glass-dock.tsx   (512 lines, SHA-256 86aebe3b84f2)
 *   deps:     framer-motion, gsap            (both now in package.json)
 *
 * THE SOURCE IS THE REGISTRY'S, UNCHANGED, except for two things this project
 * makes impossible and one the registry left out:
 *
 *   1. `@/lib/utils` does not exist here and there is no path alias, so `cn`
 *      is imported from `./cn` — the same two-line join the shadcn helper is
 *      when there is no Tailwind to merge classes for.
 *   2. `'use client'` is a Next.js directive and a no-op under Vite. Kept, so
 *      a diff against the registry stays clean.
 *   3. `useEffect` was imported and never used. This project builds with
 *      `noUnusedLocals`, so it fails the typecheck; the import is dropped and
 *      nothing else about the file changes.
 *   4. THE TOOLTIP IS REPOSITIONED. It is authored for a dock that floats at
 *      the BOTTOM of a page, so the label animates to `y: -60` — upward, clear
 *      of the icons. In a top bar that lands at y = -48, off the top of the
 *      window. It hangs below here instead. Its `x` was also `index * 52 + 12`,
 *      which does not describe its own layout (40px items, 16px gaps, 24px
 *      padding => 44 + 56i); it is measured off the element now.
 *   5. THE BUILT-IN ANIMATED ICON SET IS EMPTIED. It matches on `title`, and
 *      this app's "Home" collided with it — see the note at the call site.
 *   6. THE REGISTRY SHIPS NO CSS. It has no `css` and no `cssVars` block, the
 *      component emits bare class names (`glass-dock`, `glass-border`, `home`,
 *      `active`, ...) and a comment pointing at a `globals.css` that is not in
 *      the package. Installed as-is it renders unstyled SVG buttons. Those
 *      styles are supplied by `glass-dock.css` beside this file.
 *
 * MORPHING IS OFF, BY THE COMPONENT'S OWN DESIGN. `MorphSVGPlugin` is a paid
 * Club GreenSock plugin and is not on public npm; the component dynamically
 * imports it, catches the failure, warns once and carries on. So the icons do
 * not morph here — everything else (the spring lift, the travelling tooltip,
 * the directional blurred text swap) is the registry's and works.
 */
'use client';
...
export const GlassDock = React.forwardRef<HTMLDivElement, GlassDockProps>(
    (
        ...
        /* MEASURED, NOT ASSUMED. The original is `index * 52 + 12`, which does
           not match the layout it ships with: the items are `w-10` (40px) with
           `gap-4` (16px), so the pitch is 56 and the row starts after `px-6`
           (24px) — item i is centred at 44 + 56i, not 52i + 12. The error grows
           per item and the last of six sits about 52px from its icon. Reading
           the element removes the coupling between this arithmetic and those
           three utility classes entirely. */
        const itemRefs = useRef<(HTMLDivElement | null)[]>([]);
        const getTooltipPosition = (index: number) => {
            const el = itemRefs.current[index];
            if (!el) return index * 56 + 44;
            ...
            >
                ...
                >
                        ...
                        {hoveredIndex !== null && (
                            <motion.div
                                ...
                                animate={{
                                    opacity: 1,
                                    scale: 1,
                                    /* BELOW THE DOCK, not above it. The
                                       component is authored as a bottom-
                                       floating macOS dock, where -60 lifts the
                                       label clear of the icons. This dock is in
                                       a top bar, and -60 put the tooltip at
                                       y = -48: off the top of the window and
                                       unreadable. Measured, then flipped. */
                                    y: 62,
                            ...
                            >
                                {/* `x` is the item's CENTRE, so the pill is
                                    shifted half its own width to sit under it.
                                    A transform on the inner element, because
                                    the outer one's transform belongs to
                                    framer. */}
                                <div
                    ...
                    {items.map((el, index) => {
                        ...
                        /* THE BUILT-IN SET IS EMPTIED FOR THIS APP. Upstream this
                           is ['home','blog','marker','email','linkedin','x','github']
                           — a personal site's icons — and any item whose TITLE
                           happens to match takes a completely different branch:
                           the component's own hardcoded path, its own button, and
                           the `--tab-bar-*` custom properties, rather than the
                           icon it was handed.

                           This app's nav collides on exactly one, "Home", so five
                           destinations animated through the passed icon and Home
                           silently rendered the component's house on a different
                           code path with different styling. That asymmetry is the
                           "only Home behaves differently" bug, in both directions
                           — it was the only one that worked, then the only one
                           that did not.

                           Emptying the list sends every item through the `<Icon>`
                           branch, where `GlassDockNav` supplies an icon that
                           morphs itself. Restoring upstream behaviour is putting
                           the seven names back. */
                        const BUILT_IN_ANIMATED: string[] = [];
                        const isAnimated = BUILT_IN_ANIMATED.includes(type);
